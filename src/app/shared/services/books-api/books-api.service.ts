...
export class BooksApiService {
  ...
  private mountQuery(input: any) {
    ...
    for (let i = 0; i < properties.length; i++) {
      if (properties[i]) {
        query.push(
          `${properties[i]}=${encodeURIComponent(
            eval('input.' + properties[i])
          )}`
        );
      }
    }
  ...
  public searchBooksByTerm(term: string): Observable<Array<BookResult>> {
    ...
    return this.httpService
      .get(
        `${this.baseApiUrl}/search.json?${this.mountQuery(query)}`,
        this.headers
      )
      .pipe(map(response => response.json().docs as Array<BookResult>));
  }
}
